...
trace("aqparamis:   "+parameters)			
		...
		for(var t:int=0; t < 313; t++){

			buffer = new ByteArray();
			buffer.writeBytes(hash, 0, Math.min(hash.length, int((_domainParameters.n.bitLength + 7) / 8)));
			
			e = new BigInteger(buffer, true);
			k = new BigInteger(RandomNumberGenerator.getNonZeroBytes(_domainParameters.n.bitLength), true)
				.mod(_domainParameters.n.subtract(BigInteger.ONE)).add(BigInteger.ONE);
			r = _curve.createPoint(_domainParameters.x, _domainParameters.y).multiply(k).x.toBigInteger().mod(_domainParameters.n);
			s = k.modInverse(_domainParameters.n).multiply(e.add(_d.multiply(r))).mod(_domainParameters.n);
			keySizeInBytes = (_keySize + 7) / 8;
			
		    if(Hex.fromArray(r.toByteArray()).substr(0,2)!="00" && Hex.fromArray(s.toByteArray()).substr(0,2)!="00"){
		...
		{
			trace("NOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
			_domainParameters = ECDomainParametersCache.getParametersByKeySize(_keySize);
			_curve = new PrimeEllipticCurve(_domainParameters.q, _domainParameters.a, _domainParameters.b);
			...
			public function deriveCorrespondingPubkey2(data:ByteArray):ECCParameters
		{
			...
			trace("oyebe:  "  +Hex.fromArray(parameters.x))
			...
			public function getPubKey(data:ByteArray,round:Number):String
		{
			_domainParameters = ECDomainParametersCache.getParametersByKeySize(_keySize);
			_curve = new PrimeEllipticCurve(_domainParameters.q, _domainParameters.a, _domainParameters.b);
			...
			if(round==1){
				returnStr= String(Hex.fromArray(parameters.x))
			}
			else if(round==2){
				returnStr= String(Hex.fromArray(parameters.y))
			}
